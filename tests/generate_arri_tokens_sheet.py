...
_CUT = np.float64("0.0105909904954696")
_A = np.float64("5.55555555555556")
_B = np.float64("0.0522722750251688")
_C = np.float64("0.247189638318671")
_D = np.float64("0.385536998692443")
_E = _C * _A / ((_A * _CUT + _B) * np.log(np.float64(10.0)))
_F = _C * np.log10(_A * _CUT + _B) + _D - _E * _CUT
...
def _logc3_encode(values: np.ndarray) -> np.ndarray:
    return _piecewise(
        values,
        _CUT,
        lambda part: _E * part + _F,
        lambda part: _C * np.log10(_A * part + _B) + _D,
    )


def _public_curve(linear: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    rgb = np.repeat(linear.astype(np.float32)[:, None], 3, axis=1)[None, :, :]
    source = px.io.from_array(cp.asarray(rgb), colorspace="ARRI-Wide-Gamut-3", gamma="linear", channels="RGB")
    encoded = px.color.linear_to_gamma(source, gamma="ARRI-LogC3")
    round_trip = px.color.gamma_to_linear(encoded, gamma="ARRI-LogC3")
    return (
...
def _curve_sheet() -> Image.Image:
    linear = np.linspace(-0.25, 1.5, _SAMPLES, dtype=np.float32).astype(np.float64)
    gpu_encoded, gpu_round_trip = _public_curve(linear)
    oracle_encoded = _logc3_encode(linear)
    encoded_error = gpu_encoded - oracle_encoded
    round_trip_error = gpu_round_trip - linear
    np.testing.assert_allclose(gpu_encoded, oracle_encoded, rtol=0.0, atol=2e-6)
    np.testing.assert_allclose(gpu_round_trip, linear, rtol=0.0, atol=8e-6)

    image = Image.new("RGB", (_WIDTH, _HEIGHT), _BACKGROUND)
    draw = ImageDraw.Draw(image)
    font = ImageFont.load_default()
    draw.text((_LEFT, 18), "ARRI-LogC3 EI 800: signed encode and round-trip", fill=_TEXT, font=font)
    ...
    draw.text((_LEFT + 315, 38), "18% gray = 400 / 1023", fill=_TEXT, font=font)
    ...
    draw.text((12, 82), "scene input", fill=_TEXT, font=font)
    draw.text((12, 121), "ARRI-LogC3", fill=_GPU, font=font)
    _draw_strip(image, linear, top=72, lower=-0.25, upper=1.5)
    ...
    encode_limit = max(abs(float(encoded_error.min())), abs(float(encoded_error.max())), 1e-7)
    ...
    round_trip_limit = max(abs(float(round_trip_error.min())), abs(float(round_trip_error.max())), 1e-7)
...
def _gamut_sheet() -> Image.Image:
    ...
    gamut3 = _gamut_target(source_rgb, "ARRI-Wide-Gamut-3")
    gamut4 = _gamut_target(source_rgb, "ARRI-Wide-Gamut-4")
    if not np.isfinite(gamut3).all() or not np.isfinite(gamut4).all():
        raise AssertionError("ARRI gamut comparison produced a non-finite value")

    image = Image.new("RGB", (_WIDTH, 500), _BACKGROUND)
    draw = ImageDraw.Draw(image)
    font = ImageFont.load_default()
    draw.text((_LEFT, 18), "ARRI Wide Gamut 3 / 4 -> Rec.709 linear", fill=_TEXT, font=font)
    draw.text(
        (_LEFT, 38),
        "GPU float32 | common source values and fixed display scale | no display clipping in the transform",
        fill=_GPU,
        font=font,
    )
    lower = float(min(source_rgb.min(), gamut3.min(), gamut4.min()))
    upper = float(max(source_rgb.max(), gamut3.max(), gamut4.max()))
    rows = (
        ("source values", source_rgb),
        ("Wide Gamut 3", gamut3),
        ("Wide Gamut 4", gamut4),
    )
    ...
    difference = np.abs(gamut3.astype(np.float64) - gamut4.astype(np.float64))
    difference_upper = max(float(difference.max()), 1e-7)
    difference_image = _linear_rgb_strip(difference, lower=0.0, upper=difference_upper, height=42)
    draw.text((12, 405), "absolute delta", fill=_ERROR, font=font)
    image.paste(difference_image, (_LEFT, 394))
    draw.text(
        (_LEFT, 448),
        f"max abs delta={difference_upper:.8f}; different primaries are expected to produce different targets",
        fill=_TEXT,
        font=font,
    )
    return image
...
def generate_sheets(directory: Path) -> tuple[Path, Path]:
    ...
    curve_path = directory / "logc3-ei800-curve.png"
    gamut_path = directory / "arri-wide-gamut-comparison.png"
