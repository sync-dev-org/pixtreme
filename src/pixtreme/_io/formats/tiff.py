...
from pixtreme._io.orientation import _oriented_dimensions, _read_exif_orientation
...
def _tiff_profile(stream: BinaryIO, endian: str, count: int, field: bytes) -> bytes:
    if count <= 4:
        return field[:count]
    offset = struct.unpack(f"{endian}I", field)[0]
    current = stream.tell()
    try:
        stream.seek(offset)
        return _read_exact(stream, count)
    finally:
        stream.seek(current)
...
def _parse_tiff(source: Path | bytes) -> ImageHeader:
    with _binary_stream(source) as stream:
        ...
        icc_entry: tuple[int, int, bytes] | None = None
        duplicate_icc = False
        for _ in range(entry_count):
            entry = _read_exact(stream, 12)
            tag, field_type, count = struct.unpack(f"{endian}HHI", entry[:8])
            if tag == 34675:
                if icc_entry is None and not duplicate_icc:
                    icc_entry = (field_type, count, entry[8:12])
                else:
                    icc_entry = None
                    duplicate_icc = True
            elif tag != 274:
        ...
        elif icc_entry is None:
            carrier = _IccCarrier(present=False, profile=None)
        else:
            field_type, count, field = icc_entry
            if field_type != 7 or count > _MAX_PROFILE_SIZE:
                carrier = _IccCarrier(present=True, profile=None)
            else:
                try:
                    carrier = _IccCarrier(present=True, profile=_tiff_profile(stream, endian, count, field))
    ...
    if photometric in (0, 1) and samples == 1:
        ...
        labels = ("Y", "A")
    elif photometric == 2 and samples == 4:
        labels = ("R", "G", "B", "A")
    elif photometric == 2 and samples == 3 or photometric in (3, 6):
        labels = ("R", "G", "B")
    else:
    ...
    with _binary_stream(source) as stream:
        orientation = _read_exif_orientation(stream, description="TIFF first IFD")
